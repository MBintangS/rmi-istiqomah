...
import { Badge, Button, Input, Label, Modal, RichTextContent, Select, Spinner, Textarea } from "@/components/ui";
...
export function AdminArtikelForm({ mode, initial }: AdminArtikelFormProps) {
  ...
  const previewStatus = watch("status");
  ...
  const openPreview = () => {
    ...
    if (!values.title.trim() && !values.content.trim()) {
      toast.error("Isi judul atau konten dulu untuk melihat preview");
      return;
    }
    setPreviewOpen(true);
  };
      ...
      >
        <article className="space-y-5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant={previewStatus === "published" ? "success" : "warning"}>
              {previewStatus === "published" ? "published" : "draft"}
            </Badge>
          </div>

          <h3 className="font-display text-xl font-semibold tracking-tight text-heading sm:text-2xl">
            {previewTitle.trim() || "Tanpa judul"}
          </h3>
          ...
          <div className="relative aspect-video overflow-hidden rounded-rmi border border-foreground/10 bg-surface">
            <Image
              ...
              alt={previewTitle.trim() || "Preview thumbnail"}
              fill
              className="object-cover"
          ...
          {previewContent.trim() ? (
            <RichTextContent html={previewContent} />
          ) : (
            <p className="text-sm text-foreground/55">Konten masih kosong.</p>
          )}
        </article>
      </Modal>
    </>
